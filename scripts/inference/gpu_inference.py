...
def run_gpu_inference(model, data_loader, num_samples=10, task_type='classification', 
                     precision='mixed', cuda12_optimizations=True):
    """
    GPUを使用して高速に推論を実行
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(data_loader):
            ...
            if task_type == 'classification' and result['confidence'] is not None:
                print(f"  予測: クラス {result['prediction']} (実際: {result['target']}) - "
...
def process_inference(model, data, target, task_type):
    ...
    if hasattr(model, 'explain_prediction'):
        ...
        nt_levels = (model.get_neuromodulator_levels() 
                    if hasattr(model, 'get_neuromodulator_levels') else {})
...
def visualize_results(results, save_dir='results/gpu_inference'):
    ...
    if results[0]['neurotransmitter_levels']:
        ...
        for i, result in enumerate(results[:5]):
            ...
            for j, bar in enumerate(bars):
                val = values[j]
                color = plt.cm.Blues(abs(val/1.0)) if keys[j] == 'gaba' else plt.cm.Reds(abs(val/1.0))
                bar.set_color(color)
    ...
    for i, result in enumerate(results[:5]):
        ...
        plt.title(title, color='green' if result['prediction'] == result['target'] else 'red')
        plt.axis('off')
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=32,
                      help='バッチサイズ')
    parser.add_argument('--cuda12-optimizations', action='store_true',
