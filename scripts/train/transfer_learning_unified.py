...
from biokan.utils.cuda_info_manager import print_cuda_info, get_device, setup_japanese_fonts
...
def parse_arguments():
    ...
    parser.add_argument('--num-blocks', type=int, default=3,
                      help='BioKANブロック数')
    parser.add_argument('--attention-type', type=str, default='biological',
    ...
    parser.add_argument('--freeze-layers', type=int, default=2,
                      help='固定する層の数')
    parser.add_argument('--save-dir', type=str, default='results/transfer_learning',
                      help='結果の保存ディレクトリ')
...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % 100 == 0:
            print(f'エポック: {epoch} [{batch_idx * len(data)}/{len(train_loader.dataset)} '
...
def main():
    ...
    # データセットの読み込み
    train_loader, test_loader, in_channels, num_classes = load_dataset(args.dataset, args.batch_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), os.path.join(args.save_dir, 'best_transfer_model.pth'))
    
    # 訓練履歴の可視化
    plot_training_history(train_losses, train_accs, test_losses, test_accs, args.save_dir)
