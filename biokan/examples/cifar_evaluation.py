"""
CIFAR-10データセットでBioKANモデルを評価するスクリプト
"""
...
def explain_predictions(model: nn.Module, 
    ...
    for i, (input_data, target) in enumerate(samples):
        # 入力データを前処理
        input_data_flat = input_data.view(-1)  # [3*32*32]
        input_tensor = input_data_flat.unsqueeze(0).to(device)  # [1, 3*32*32]
        ...
        explanation['target_name'] = classes[target.item()]
        explanation['predicted'] = predicted.item()
        explanation['predicted_name'] = classes[predicted.item()]
        
        # 反事実的説明（異なるクラスに変更する）
        if predicted.item() == target.item():
            # 正解の場合、異なるクラスを目標に設定
            other_class = (target.item() + 1) % 10
            ...
            explanation['counterfactual_target_name'] = classes[other_class]
        ...
        # 図を保存（オプション）
        if save_path:
            sample_dir = f"{save_path}/sample_{i}_{classes[target.item()]}"
            os.makedirs(sample_dir, exist_ok=True)
            
            # 元画像を保存
            plt.figure(figsize=(3, 3))
            img = input_data.cpu().numpy().transpose(1, 2, 0)  # [3, 32, 32] -> [32, 32, 3]
            # 標準化を元に戻す
            img = img * np.array([0.2023, 0.1994, 0.2010]) + np.array([0.4914, 0.4822, 0.4465])
            img = np.clip(img, 0, 1)
            plt.imshow(img)
            plt.title(f"実際: {classes[target.item()]}\n予測: {classes[predicted.item()]}")
            plt.axis('off')
            plt.tight_layout()
...
def main():
    # コマンドライン引数のパース
    parser = argparse.ArgumentParser(description='BioKANを使用したCIFAR-10分類器の訓練と評価')
    ...
    parser.add_argument('--hidden_dim', type=int, default=256, help='隠れ層の次元')
    ...
    parser.add_argument('--save_dir', type=str, default='./outputs_cifar', help='出力ディレクトリ')
    ...
    model_save_path = os.path.join(args.save_dir, 'biokan_cifar10.pth')
