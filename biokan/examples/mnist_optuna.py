"""
MNISTデータセットでOptunaを使ったBioKANモデルのハイパーパラメータ最適化
"""
...
def objective(trial: Trial, device: torch.device, data_loaders: Dict[str, DataLoader], 
             n_epochs: int = 10, save_dir: str = "./optuna_results") -> float:
    """
...
def evaluate_best_model(model: nn.Module, test_loader: DataLoader, device: torch.device) -> Dict[str, float]:
    """
    最良のモデルをテストデータで評価
...
def main():
    # コマンドライン引数のパース
    parser = argparse.ArgumentParser(description='MNISTデータセットでOptunaを使ったBioKANモデルのハイパーパラメータ最適化')
    parser.add_argument('--batch_size', type=int, default=128, help='バッチサイズ')
    parser.add_argument('--epochs', type=int, default=10, help='各トライアルのエポック数')
    parser.add_argument('--n_trials', type=int, default=20, help='Optunaのトライアル数')
    parser.add_argument('--save_dir', type=str, default='./optuna_mnist', help='結果の保存先ディレクトリ')
